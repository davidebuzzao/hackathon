...
count_file <- file.path(data, "count_matrix/count_matrix_pancreas.txt")
countdata <- read.table(file = count_file, sep = ' ', header = TRUE)

# Read input data
colData_file <- file.path(data, "pancreas.csv")
...
# Other possible initial exploratory design includes: Sex, Age.Bracket, Hardy.Scale, Pathology.Categories_pancreas
...
library("dplyr") 
library("ggplot2")
pdf(file = "Exploratory_Analysis_pancreas.pdf")
...
rownames(sampleDistMatrix) <- paste(vsd$SAMPID_pancreas) 
...
pdf(file = "PCA_pancreas_Countstringent.pdf")
...
gpca.dat$Pathology.Categories_pancreas <- dds$Pathology.Categories_pancreas
ggplot(gpca.dat, aes(x = dim1, y = dim2, color = Pathology.Categories_pancreas)) + 
...
ggplot(mds, aes(x = `1`, y = `2`, color = Pathology.Categories_pancreas)) + 
    geom_point(size = 3) + coord_fixed() + ggtitle("MDS with VST data")

# PoissonDistance
mdsPois <- as.data.frame(colData(dds)) %>% cbind(cmdscale(samplePoisDistMatrix))
ggplot(mdsPois, aes(x = `1`, y = `2`, color = Pathology.Categories_pancreas)) + 
...
pdf(file = "DEresults_pancreas.pdf")
